          ...
          <div class="col-md-8 col-xs-12 q-mb-sm">
            <div class="row q-col-gutter-md">
              ...
              <div class="col-md-8 col-xs-12">
                <q-select class="q-mb-sm" filled v-model="modeloInput" use-input input-debounce="1000" label="Elige el modelo"
                  :options="modelosDeAuto" @filter="filterFn2" behavior="menu" :loading="loading2" :disable="loading2">
                  <template v-slot:prepend><q-icon name="img:statics/icons/icon-two.png"></q-icon></template>
                  <template v-slot:no-option>
                    <q-item><q-item-section class="text-grey">No hay resultados</q-item-section></q-item>
                  </template>
                  <template v-slot:append><q-icon class="cursor-pointer" @click.stop="getProductos" outline round color="red" name="search" /></template>
                </q-select>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    getProductos(e) {
      e.preventDefault()
      this.$router.push({ name: 'Panel-Productos-Search', params: { modelo: this.modelos.find(item => item.name == this.modeloInput).slug } })
    },
    getProductoByCode() {
      var self = this
      fb.productos.where('slug', '==', self.codigo.toLowerCase()).get().then(function (querySnapshot) {
        ...
        if (!querySnapshot.empty) {
          querySnapshot.forEach(function (doc) {
            ...
            self.$router.push({ name: 'Panel-Edit-Product', params: { producto: docData.slug } })
          });
        } else {
